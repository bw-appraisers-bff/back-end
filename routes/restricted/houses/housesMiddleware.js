...
function validateHouseObj(req, res, next) {
  ...
  else if (house.bedrooms < 1) res.status(400).json({ message: 'must have at least one bedroom' })
  else if (!house.bathrooms) res.status(404).json({ message: 'missing bathrooms field' })
  else if (house.bathrooms < 1) res.status(400).json({ message: 'must have at least one bathroom' })
  else next()
}